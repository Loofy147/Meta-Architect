...
def test_spectral_mixing_layer():
    """Test the SpectralMixingLayer for correct shape and gradient flow."""
    d_head = 64
    grid_radius = 1
    batch_size = 2
    seq_len = 32
    ...
    # 1. Setup
    from hamha.topology import build_adjacency_matrix, generate_hex_grid
    from hamha.mixing import SpectralMixingLayer
    coords = generate_hex_grid(grid_radius)
    num_heads = len(coords)
    adjacency_matrix = build_adjacency_matrix(coords)

    # 2. Instantiate layer
    layer = SpectralMixingLayer(d_head, num_heads, adjacency_matrix)

    # 3. Dummy input
    head_outputs = [
        torch.randn(batch_size, seq_len, d_head, requires_grad=True)
        for _ in range(num_heads)
    ]

    # 4. Forward pass
    mixed_outputs = layer(head_outputs)

    # 5. Check output shape
    assert isinstance(mixed_outputs, list)
    assert len(mixed_outputs) == num_heads
    assert all(out.shape == (batch_size, seq_len, d_head) for out in mixed_outputs)

    # 6. Check for gradient flow to spectral filter
    total_output = torch.stack(mixed_outputs).sum()
    total_output.backward()

    assert layer.spectral_filter.grad is not None
    assert not torch.all(layer.spectral_filter.grad == 0)

    # 7. Verify orthogonality of eigenvectors
    U = layer.U
    identity = torch.eye(U.size(0))
    assert torch.allclose(U.T @ U, identity, atol=1e-6)
